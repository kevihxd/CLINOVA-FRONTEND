...
const MOCK_DATA = Array.from({ length: 5 }, (_, i) => ({
    ...
    fecha_modificacion: '2024-03-20 14:45:00',
    creado_por: 1,  
    modificado_por: 1
}));
        ...
        <div className="min-h-screen bg-slate-50/50 p-6 md:p-8 animate-fade-in">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-slate-800 tracking-tight">Tipo de Documento</h1>
                        <p className="text-sm text-slate-500 font-medium">Gestión de tipos de identificación</p>
                    </div>
                </div>
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="bg-slate-50/80 border-b border-slate-100">
                                <th className="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Sigla</th>
                                ...
                                <th className="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Modificación</th>
                            ...
                            {currentItems.map((item) => (
                                <tr key={item.id} className="hover:bg-blue-50/30 transition-colors group">
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <div className="flex items-center gap-3">
                                            <span className="text-sm font-bold text-slate-700 bg-slate-100 px-2.5 py-1 rounded-md border border-slate-200 min-w-[40px] text-center">
                                                {item.sigla}
                                            </span>
                                        </div>
                                    </td>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <div className="flex flex-col gap-1">
                                            <div className="flex items-center gap-1.5 text-xs text-slate-500">
                                                <Calendar className="w-3 h-3" />
                                                <span>{item.fecha_modificacion.split(' ')[0]}</span>
                                            </div>
                                            <div className="flex items-center gap-1.5 text-xs text-slate-400">
                                                <User className="w-3 h-3" />
                                                <span>ID: {item.modificado_por}</span>
                                            </div>
                                        </div>
                                    </td>
